...
export function CreateInvoiceForm({
  ...
  function applyGebuhPosition(idx: number, pos: GebuhPosition) {
    setItems((prev) => prev.map((item, i) =>
      i === idx ? {
        ...item,
        description: `GebüH ${pos.ziffer} – ${pos.kurztext}`,
        unitPrice:   pos.empfehlung_eur.toFixed(2).replace('.', ','),
        taxRate:     '0',
      } : item
    ))
    setCatalogOpenIdx(null)
      ...
      <form action={action} className="space-y-6">
        ...
        <div>
          ...
          <select
            ...
            defaultValue={defaultClientId ?? ''} required disabled={pending}
            ...
            {items.map((item, idx) => (
              <div key={idx} className="grid grid-cols-12 gap-2 items-center">
                  ...
                  {hasCatalog && (
                    <div className="relative z-50">
                      <button
                        ...
                        title="Aus Katalog oder GebüH wählen"
                ...
                <input
                  ...
                  min={0.1} step="any" required disabled={pending}
                  className={`col-span-2 ${inputCls}`}
                />
                <input
                  type="text" name="item_unitPrice"
                  value={item.unitPrice} onChange={(e) => updateItem(idx, 'unitPrice', e.target.value)}
                  placeholder="0,00" required disabled={pending}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className={isPerItem ? 'col-span-2' : 'col-span-1'}>
            ...
            <select
              id="taxMode" name="taxMode"
              value={taxMode} onChange={(e) => setTaxMode(e.target.value)}
              disabled={pending}
              className={`block w-full ${inputCls} px-3.5 py-2.5`}
            >
          ...
          {!isPerItem && taxMode !== 'none' && (
            <div>
              <label htmlFor="taxRate" className="block text-sm font-medium text-zinc-700 mb-1.5">MwSt.-Satz (%)</label>
              <input
                id="taxRate" name="taxRate" type="number"
                value={taxRate} onChange={(e) => setTaxRate(e.target.value)}
                min={0} max={100} disabled={pending}
                className={`block w-full ${inputCls} px-3.5 py-2.5`}
              />
            </div>
          )}
        ...
        {/* §14 UStG: Rechnungs- und Leistungsdaten */}
        <div className="rounded-lg border border-zinc-200 bg-zinc-50 p-4 space-y-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-zinc-400">Pflichtangaben §14 UStG</p>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="issuedDate" className="block text-sm font-medium text-zinc-700 mb-1.5">
                Rechnungsdatum
              </label>
              <input
                id="issuedDate" name="issuedDate" type="date"
                defaultValue={todayStr} disabled={pending}
                className={`block w-full ${inputCls} px-3.5 py-2.5`}
              />
            </div>
            <div>
              ...
              <input
                ...
                defaultValue={defaultDueStr} disabled={pending}
                className={`block w-full ${inputCls} px-3.5 py-2.5`}
              />
            </div>
            <div>
              <label htmlFor="servicePeriodFrom" className="block text-sm font-medium text-zinc-700 mb-1.5">
                Leistungszeitraum von
              </label>
              <input
                id="servicePeriodFrom" name="servicePeriodFrom" type="date"
                disabled={pending}
                className={`block w-full ${inputCls} px-3.5 py-2.5`}
              />
            </div>
            <div>
              <label htmlFor="servicePeriodTo" className="block text-sm font-medium text-zinc-700 mb-1.5">
                Leistungszeitraum bis
              </label>
              <input
                id="servicePeriodTo" name="servicePeriodTo" type="date"
                disabled={pending}
                className={`block w-full ${inputCls} px-3.5 py-2.5`}
              />
            </div>
          ...
          <p className="text-xs text-zinc-400">
            Der Leistungszeitraum ist Pflicht, wenn er vom Rechnungsdatum abweicht (§14 Abs.4 Nr.6 UStG).
          </p>
        </div>
